## dec_avc_au

How to use the `Transcoder.Push` method to decode a sequence of H.264 Access Units to YUV uncompressed file.   
   

### Command Line

```sh
dec_avc_au --input <directory> --output <file.yuv> [--color <COLOR>]
```

###	Examples

List options:

```sh
./bin/net10.0/dec_avc_au --help
dec_avc_au 1.0.0.0
Copyright (C) 2025 Primo Software

  -i, --input     input directory containing H.264 Access Unit files

  -o, --output    output YUV file

  -c, --color     output color format (e.g. yuv420, yv12, nv12)

  --colors        list supported color formats

  --help          Display this help screen.
```

List supported color formats:

```sh
./bin/net10.0/dec_avc_au --colors
```

Decode the H.264 Access Units from `./assets/vid/foreman_qcif.h264.au/` into output file `./output/dec_avc_au/foreman_qcif.yuv`:

```sh
# Linux and macOS 
mkdir -p ./output/dec_avc_au

./bin/net10.0/dec_avc_au \
    --input ./assets/vid/foreman_qcif.h264.au \
    --output ./output/dec_avc_au/foreman_qcif.yuv \
    --color yuv420
```

```powershell
# Windows
mkdir -Force -Path ./output/dec_avc_au

./bin/net10.0/dec_avc_au `
    --input ./assets/vid/foreman_qcif.h264.au `
    --output ./output/dec_avc_au/foreman_qcif.yuv `
    --color yuv420
```
