...
namespace CliSample
{
    class Options
    {
        // mux_webm_file options
        [Option('a', "audio", HelpText = "input Vorbis/WebM files. Could be a list of files.")]
        public IEnumerable<string> AudioFiles { get; set; }

        [Option('v', "video", HelpText = "input VP8/WebM files. Could be a list of files.")]
        public IEnumerable<string> VideoFiles { get; set; }

        [Option('o', "output", HelpText = "output WebM file")]
        ...
        void SetDefaultOptions()
        {
            ...
            string videoFile = Path.Combine(exeDir, "../../assets/vid/big-buck-bunny_trailer_vp8_vorbis.vid.webm");
            VideoFiles = new List<string> { videoFile };

            string audioFile = Path.Combine(exeDir, "../../assets/aud/big-buck-bunny_trailer_vp8_vorbis.aud.webm");
            AudioFiles = new List<string> { audioFile };

            OutputFile = Path.Combine(exeDir, "../../output/mux_webm_file/big-buck-bunny_trailer.webm");

            Console.WriteLine("Using default options: ");
            Console.Write("mux_webm_file");
